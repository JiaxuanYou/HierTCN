...
class Dense(base.Layer):
  ...
  def build(self, input_shape):
    ...
    self.kernel = self.add_variable('kernel',
                                    ...
                                    dtype=self.dtype,
                                    trainable=True)
    if self.has_weightnorm:
        g = tf.get_variable('g', shape=[self.units], dtype=tf.float32, initializer=self.kernel_initializer, trainable=True)
        self.kernel = g * self.kernel / (tf.sqrt(tf.reduce_sum(tf.square(self.kernel), [0], keep_dims=True)))
    if self.use_bias:
