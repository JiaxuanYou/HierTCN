...
class _Conv(base.Layer):
  ...
  def build(self, input_shape):
    ...
    if self.has_weightnorm:
        g = tf.get_variable('g', shape=[self.filters], dtype=tf.float32, initializer=self.kernel_initializer, trainable=True)
        self.kernel = tf.reshape(g, [1, 1, self.filters]) * tf.nn.l2_normalize(self.kernel, [0,1])
    if self.use_bias:
